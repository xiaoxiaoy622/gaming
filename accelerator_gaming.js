...
(function () {
    ...
    function _mq(breakpoint, scale) {
        ...
        return '@media (max-width:' + breakpoint + 'px){' +
            ...
            '.gaming-panel{top:' + pt + 'px;left:' + ptl + 'px;width:' + pw + 'px;padding:' + pad + 'px;clip-path:polygon(' + clipR + 'px 0,calc(100% - ' + clipR + 'px) 0,100% ' + clipR + 'px,100% calc(100% - ' + clipR + 'px),calc(100% - ' + clipR + 'px) 100%,' + clipR + 'px 100%,0 calc(100% - ' + clipR + 'px),0 ' + clipR + 'px)}' +
            '.gp-header{font-size:' + hFS + 'px}' +
            ...
            '.gaming-grid{grid-template-columns:repeat(' + cols + ',1fr);gap:' + gap + 'px}' +
    ...
    var _cssText = [
        ...
        // --- 游戏面板（基准值 s=1.0）---
        '.gaming-panel{position:fixed;top:' + _BASE.panelTop + 'px;left:' + _BASE.pillTL + 'px;width:' + _BASE.panelW + 'px;background:#1a1a2e;border:2px solid #00ff88;z-index:2147483646;user-select:none;visibility:hidden;opacity:0;transform:translateY(10px);transition:visibility 0s .2s,opacity .2s,transform .2s;clip-path:polygon(' + _BASE.clipR + 'px 0,calc(100% - ' + _BASE.clipR + 'px) 0,100% ' + _BASE.clipR + 'px,100% calc(100% - ' + _BASE.clipR + 'px),calc(100% - ' + _BASE.clipR + 'px) 100%,' + _BASE.clipR + 'px 100%,0 calc(100% - ' + _BASE.clipR + 'px),0 ' + _BASE.clipR + 'px);padding:' + _BASE.pad + 'px}',
        '.gaming-panel.open{visibility:visible;opacity:1;transform:translateY(0);transition:visibility 0s,opacity .2s,transform .2s}',
        '.gp-header{font-family:"Press Start 2P",monospace;font-size:' + _BASE.headerFS + 'px;color:#00ff88;letter-spacing:2px;margin-bottom:12px;padding-bottom:8px;border-bottom:1px solid #2d2d44;text-shadow:0 0 6px rgba(0,255,136,.4)}',
    ...
    function _mountUI() {
        ...
        sliderWrap.innerHTML = '<label>精确调速</label><input type="range" class="gaming-slider" id="gamingSlider" min="0" max="100" value="30">';
        ...
        // 拖拽结束后标记，防止 click 事件触发
        var _pillDragEnded = false;
        _pill.addEventListener('touchend', function () { _pillDragEnded = true; });
        _pill.addEventListener('mouseup', function () { _pillDragEnded = true; });
        document.addEventListener('touchstart', function () { _wasDragged = _pillDragEnded; _pillDragEnded = false; }, { passive: true });
        document.addEventListener('mousedown', function () { _wasDragged = _pillDragEnded; _pillDragEnded = false; }, { passive: true });
        ...
        function _repositionPanel() {
            ...
            var ph = _panel.offsetHeight || 520;
            var pillH = _pill.offsetHeight || _BASE.pillH;

            // 检测横屏（宽高比 < 1 或高度明显小于宽度）
            ...
            // 面板位置：横屏时显示在胶囊右侧，竖屏时显示在下方
            var panelX, panelY;
            if (_isLandscape) {
                // 横屏：面板在胶囊右侧，垂直居中
                panelX = cx + pillH + 8;
                panelY = cy + (pillH - ph) / 2;
            } else {
                // 竖屏：面板在胶囊下方
                panelX = cx;
                panelY = cy + pillH + 8;
