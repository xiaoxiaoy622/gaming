...
(function () {
    ...
    var _MIN_PANEL_W = 160;   // 面板最小宽度
    var _MIN_PILL_H = 36;     // 胶囊最小高度（触控友好）
    var _MIN_PILL_W = 54;     // 胶囊最小宽度（确保图标+文字可读）
    var _MIN_BTN_H  = 28;     // 按钮最小高度（触控友好）

    // 所有元素的基准值（s=1.0 对应 1366px）
    var _BASE = {
        pillW: 70, pillH: 36, pillTL: 30,   // 触发器：宽/高/上左边距
    ...
    // --- 断点与缩放比（基于主流设备屏幕宽度）---
    // 桌面:     1366px+  s=1.0
    // 小笔记本: 1200px   s≈0.878  面板约 263px
    // 小窗:     1024px   s≈0.750  面板约 225px
    // 平板:     768px    s≈0.562  面板约 169px
    // 大屏手机: 480px    s≈0.351  面板约 160px
    // 手机:     390px    s≈0.285  面板约 160px（clamp）
    // 小屏手机: 320px    s≈0.238  面板约 160px（clamp）
    var _SCALE_TABLE = {
        wideMonitor: 1200 / _BASE_WIDTH,
        monitor:     1024 / _BASE_WIDTH,
        tablet:      768 / _BASE_WIDTH,
    ...
    // 胶囊宽度：不低于最小可读宽度
    function _pillW(scale) {
        return Math.max(_MIN_PILL_W, _sz(_BASE.pillW, scale));
    }
    // 按钮高度：不低于最小触控安全区
    ...
    function _getScale() {
        ...
        if (w >= 1200) return _SCALE_TABLE.wideMonitor;
        if (w >= 1024) return _SCALE_TABLE.monitor;
        if (w >= 768)  return _SCALE_TABLE.tablet;
        if (w >= 480)  return _SCALE_TABLE.lgPhone;
        if (w >= 390)  return _SCALE_TABLE.phone;
        if (w >= 320)  return _SCALE_TABLE.small;
        return _SCALE_TABLE.small;
    }
    // 获取当前断点名称
    function _getBreakpoint() {
        var w = window.innerWidth;
        if (w >= 1366) return 'desktop';
        if (w >= 1200) return 'wideMonitor';
        if (w >= 1024) return 'monitor';
    ...
    function _gridCols() {
        ...
        if (bp === 'desktop' || bp === 'wideMonitor') return 4;
        if (bp === 'monitor' || bp === 'lgPhone' || bp === 'tablet') return 3;
        return 2;
    }

    // 生成媒体查询 CSS 字符串（bpPx = 断点像素值）
    function _mq(bpPx, scale) {
        var b = _BASE, s = scale;
        ...
        var cols = (bpPx >= 1200) ? 4
                 : (bpPx >= 480)  ? 3
                 : 2;
        // 移动端紧凑模式：480px 及以下缩小内边距和间距
        var isCompact = (bpPx <= 480);
        ...
        return '@media (max-width:' + bpPx + 'px){' +
            '.gaming-pill{top:' + ptl + 'px;left:' + ptl + 'px;width:' + _pillW(s) + 'px;height:' + ph + 'px}' +
            '.pill-icon{border-left-width:' + iL + 'px;border-top-width:' + iTB + 'px;border-bottom-width:' + iTB + 'px}' +
            '.pill-speed{font-size:' + spFS + 'px}' +
            '.gaming-panel{top:' + pt + 'px;left:' + ptl + 'px;width:' + pw + 'px;max-width:calc(100vw - 16px);padding:' + cPad + 'px;clip-path:polygon(' + clipR + 'px 0,calc(100% - ' + clipR + 'px) 0,100% ' + clipR + 'px,100% calc(100% - ' + clipR + 'px),calc(100% - ' + clipR + 'px) 100%,' + clipR + 'px 100%,0 calc(100% - ' + clipR + 'px),0 ' + clipR + 'px);overflow-y:auto}' +
            '.gp-header{font-size:' + hFS + 'px;margin-bottom:' + cMbtn + 'px;padding-bottom:' + cPbot + 'px}' +
            '.gp-close{width:' + Math.max(24, Math.round(hFS * 1.6)) + 'px;height:' + Math.max(24, Math.round(hFS * 1.6)) + 'px;font-size:' + hFS + 'px}' +
            ...
            '.gaming-reset{padding:' + rPV + 'px ' + rPH + 'px;font-size:' + rFS + 'px;min-width:40px;min-height:24px;clip-path:polygon(' + btnClip + 'px 0,calc(100% - ' + btnClip + 'px) 0,100% ' + btnClip + 'px,100% calc(100% - ' + btnClip + 'px),calc(100% - ' + btnClip + 'px) 100%,' + btnClip + 'px 100%,0 calc(100% - ' + btnClip + 'px),0 ' + btnClip + 'px)}' +
    ...
    var _cssText = [
        ...
        '.gaming-slider-wrap label{font-size:10px;color:#8888aa;display:block;margin-bottom:6px;text-align:center}',
        ...
        // --- 底部 ---
        '.gaming-footer{display:flex;align-items:center;justify-content:space-between;padding-top:10px;border-top:1px solid #2d2d44;position:sticky;bottom:0;background:#1a1a2e;z-index:1}',
        ...
        '.gaming-persist-label{font-size:' + _BASE.persistFS + 'px;color:#8888aa;display:inline-flex;align-items:center;gap:6px;white-space:nowrap;line-height:1;vertical-align:middle}',
        '.gaming-persist-label input{accent-color:#00ff88;margin:0;width:14px;height:14px;flex-shrink:0}',
        ...
        // --- 响应式断点（从宽到窄，CSS 级联后者覆盖前者）---
        // wideMonitor 1200px → s≈0.878  面板 263px
        // monitor     1024px → s≈0.750  面板 225px
        // tablet       768px → s≈0.562  面板 169px
        // lgPhone      480px → s≈0.351  面板 160px（clamp）
        // phone        390px → s≈0.285  面板 160px（clamp）
        // small        320px → clamp    面板 160px
        _mq(1200, _SCALE_TABLE.wideMonitor),
        _mq(1024, _SCALE_TABLE.monitor),
        _mq(768,  _SCALE_TABLE.tablet),
        _mq(480,  _SCALE_TABLE.lgPhone),
        _mq(390,  _SCALE_TABLE.phone),
        _mq(320,  _SCALE_TABLE.small),
    ...
    var _SPEEDS = [0.1, 1, 3, 5, 10, 20, 30, 50, 80, 100, 150, 200];
    ...
    function setSpeedUI(v, highlightBtns) {
        ...
        _currentSpeed = v;
        var label = v >= 1 ? (Math.round(v * 10) / 10) : (Math.round(v * 100) / 100);
    ...
    function _mountUI() {
        ...
        _slider.addEventListener('input', function () {
            ...
            var label = rawSpeed >= 1 ? (Math.round(rawSpeed * 10) / 10) : (Math.round(rawSpeed * 100) / 100);
